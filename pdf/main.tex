...
\begin{longtable}{|p{16cm}|}
    \textbf{RESUMEN}:\\
    El objetivo de este proyecto ha sido realizar un programa que pueda realizar múltiples funciones útiles para \\
    \hline
\end{longtable}
...
\item[MLP]: \textit{Multilayer Perceptron} es una red neuronal con sus nodos totalmente conectados entre sí. La entrada es insertada en la red, atraviesa una o varias capas de nodos ocultas y se transforma en otro dato hasta su salida en la última capa. El objetivo es construir relaciones complejas entre la entrada y la salida.

\item[PCA]: \textit{Principal Component Analysis} es una técnica que reduce el número de características manteniendo la máxima información. PCA usa álgebra lineal para estandarizar los datos y seleccionar dónde hay un cambio de valor más amplio, que es donde se ubican los datos más valiosos.

\item[Puntos locales extremos]: Denominado también \textit{Local extrema}, son valores mínimos y máximos en una curva o trazo que cambia de dirección en múltiples ocasiones. Estos se detectan comprobando si su valor en una imagen es menor o mayor que el de todos sus píxeles colindantes.
...
La Inteligencia Artificial se está implementando actualmente en cualquier trabajo o proyecto sin ningún tipo de control y sin importar si es realmente necesario. Un ejemplo claro es el artículo de investigación de Gina Belmonte et Al.\cite{BELMONTE2025103154} en el que entrenan un modelo para segmentar tejido cerebral, concretamente para aplicaciones de neuroimagen y radioterapia. Otro ejemplo de tarea muy importante actualmente, debido al crecimiento de la industria de coches autónomos, es la detección e identificación de vehículos en tiempo real, donde unos sensores se encargan de detectar otros vehículos en las proximidades para poder razonar y actuar gracias a esta informacion, como puede ser al aparcar, puedan detectar y medir distancias a un vehículo próximo. Y no sólo es importante para los coches autónomos también lo es para:
\begin{itemize}
    ...
    \item \textbf{Coches robados:} Simplemente se puede hacer una deteccion de los coches y compararlos uno a uno con una base de datos de coches (o vehículos en general) robados y poder realizar avisos a las autoridades.
\end{itemize}

Este tipo de programas son una realidad muy común ya, con múltiples investigaciones y proyectos en partes del mundo variadas como China o India para algunas de las implementaciones de detección de vehículos y/o matrículas Un ejemplo claro puede ser el caso del proyecto de Sarvesh Shirulkar et Al.\cite{10984828} con YOLO para controlar las senales de tráfico según las detecciones del modelo YOLO. Otro ejemplo es el de Anning Ji y Xintao Ma\cite{JI2025804}, en el que utilizan como base un modelo YOLO10 al que se le modifica la parte de extraccion de características. Aunque estos modelos predominen la eleccion de investigadores, hay otras opciones con diferentes arquitecturas que pueden ser muy útiles en escenarios similares, como puede ser la red neuronal de Leyre Encio et Al.\cite{articleEncio} basada en CenterNet, que trata de reproducir un diseño similar a la de los transformadores con su codificador-decodificador.

\section{Objetivos}

El objetivo principal de este proyecto es la realización de un programa que pueda ayudar en gran medida a la gestión del tráfico urbano mediante la detección, reconocimiento y lectura de vehículos y matrículas. Este sistema debe realizar estas tareas en un tiempo ínfimo (en cuestión de centésimas de segundo) y con una gran precisión

Los objetos secundarios son:
\begin{itemize}
    \item Analizar el estado del arte en la materia de
\end{itemize}
...
Por último, en el capítulo \ref{Conclusión} o \textbf{Conclusión} se discuten los problemas encontrados durante el desarrollo del proyecto, las posibles ampliaciones aplicables, opiniones personales sobre este trabajo y los agradecimientos.
...
En la mayoría de artículos, los investigadores usan alguno de los tantos modelos YOLO existentes. Ya que este tipo de modelos destaca en cualquier tipo de tarea gracias a su flexibilidad, es especialmente prominente a dar buenos resultados en la tarea de deteccion y reconocimiento tanto de vehículos como de matrículas.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        & & & & \textbf{social (1)} & \\
        \hline
        Ingeniero IA & 140 h & 30,26€/h & 4236,4€ & 266,89€ & 999,79€\\ 
        \hline
        Desarrollador Web & . & . & . & . & .\\
        \hline
        \multicolumn{3}{|c|}{\textbf{Coste total}} & \textbf{.} & \textbf{.} & \textbf{.}\\
        \hline
    \end{tabular}
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|}
        ...
        Coste de personal & .\\
        \hline
        {\textbf{Coste total del proyecto}} & \textbf{.}\\
        \hline
    \end{tabular}
    \label{Coste-Total}
\end{table}
