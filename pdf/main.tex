...
\begin{longtable}{|p{16cm}|}
    \textbf{RESUMEN}:\\
    El objetivo de este proyecto ha sido realizar un programa que pueda realizar múltiples funciones útiles para ayudar a la gestión del tráfico urbano, comenzando con la base de: (1) detección y reconocimiento de vehículos y (2) detección y lectura de matrículas. Esta tarea es realizada a partir de archivos (vídeos e imágenes) y múltiples cámaras, que el programa lee, reconoce y procesa para tener los resultados de la detección. El estado del arte en este campo es muy extenso, con multitud de implementaciones sencillas utilizando mayoritariamente modelos YOLO junto con tecnologías OCR como EasyOCR, por lo que en cuestión de rendimiento, se podrán realizar comparaciones rápidas con otros proyectos y programas. Para realizar estas tareas,se han escogido dos modelos de detección con implementaciones completamente diferentes: el modelo YOLOv26n (el más pequeño de su última versión) y el modelo RT-DETR, un modelo novedoso que trata de equiparar la velocidad del modelo YOLO usando transformadores aplicados a detección de objetos. Para leer los caracteres de las matrículas se ha escogido la tecnología TesseractOCR, la cual destaca en todos los ámbitos gracias a su precisión. Para comparar los dos métodos de forma equitativa, se han comenzado por recolectar varios conjuntos de datos de vehículos y matrículas, se han reorganizado para poder ser procesados por los modelos y se han puesto a entrenar con diferentes configuraciones. Gracias al uso de métricas comunes para ambos, como mAP50 y mAp50-95, se ha podido extraer que el método que mejores resultados provee es el modelo YOLOv26n, para ambas tareas de detección, debido a su alta precisión y su tiempo de procesamiento, que lo hacen apto para su uso en vídeos y\\ múltiples cámaras en tiempo real. Los modelos seleccionados, entrenados y validados se introducen en un programa desarrollado en Python completamente con la librería Streamlit para desplegar esta "página web" y se accede a través de un navegador web. Los resultados obtenidos validan la eficacia del sistema y abren la puerta a futuras mejoras, como el entrenamiento con datos con imperfecciones (como oclusiones, mala iluminación,...), la ampliación a dispositivos móviles y otras implementaciones que ayuden a las labores de gestión del tráfico, como reconocer la dirección de un vehículo.\\
    \hline
\end{longtable}
...
\begin{longtable}{|p{16cm}|}    
    \textbf{ABSTRACT}:\\
    The goal of this project was to develop a program capable of performing multiple useful functions to assist with urban traffic management, based on: (1) vehicle detection and recognition, and (2) license plate detection and reading. This task is carried out using files (videos and images) and multiple cameras, which the program reads, recognizes, and processes to generate detection results. The state of the art in this field is very extensive, with a multitude of simple implementations that primarily use YOLO models alongside OCR technologies such as EasyOCR; therefore, in terms of performance, quick comparisons can be made with other projects and programs. To perform these tasks, two detection models with completely different implementations have been chosen: the YOLOv26n model (the smallest in its latest version) and the RT-DETR model, a novel model that aims to match the speed of the YOLO model by using transformers applied to object detection. To read the characters on license plates, we have chosen TesseractOCR technology, which excels in all areas thanks to its accuracy. To compare the two methods fairly, we began by collecting several datasets of vehicles and license plates, reorganized them so they could be processed by the models, and trained them using different configurations. By using common metrics for both, such as mAP50 and mAP50-95, we were able to determine that the YOLOv26n model provides the best results for both detection tasks, due to its high accuracy and processing speed, which make it suitable for use with videos and multiple cameras in real time. The selected, trained, and validated models are integrated into a program developed entirely in Python using the Streamlit library to display this “web page,” which is accessed via a web browser. The results obtained validate the system’s effectiveness and pave the way for future improvements, such as training with imperfect data (e.g., occlusions, poor lighting, etc.), expansion to mobile devices, and other implementations that aid in traffic\\ management tasks, such as recognizing a vehicle’s direction.\\
    \hline
...
\item[AP] \textit{Average Precision}, es una métrica de detección de objetos que cuantifica la precisión del modelo a la hora de detectar los objetos correctamente con la menor cantidad de falsos positivos posible.

\item[Bounding Box] Las cajas delimitadoras son un marco rectangular trazado alrededor de un objeto de interés en una imagen, como puede ser un objeto o un animal. En tareas abordadas por modelos como YOLO, los recuadros delimitadores representan la ubicación y el tamaño correcto de los objetos en los datos de entrenamiento, que enseña al modelo a localizar objetos con precisión \footnote{Definición extraída de \url{https://www.ultralytics.com/es/glossary/bounding-box}}.

\item[CNN]: En Inglés \textit{Convolutional Neural Network} o Redes Neuronales Convolucionales, es un tipo de red neuronal profunda diseñada para tareas que requieren reconocimiento de objetos, como clasificación de imágenes, segmentación y detección de estos a partir de imágenes.

\item[Data Augmentation] El aumento de datos es el suministro adicional de datos en \textit{Deep Learning} para aumentar el tamaño y variabilidad del conjunto de datos.

\item[Dataset]: Es un conjunto de datos cuyo contenido es específico de una tarea en concreto. Los datasets son uno de los integrantes más importantes de la Inteligencia Artificial, conteniendo toda la información que la aplicación necesita para aprender, validar y probar su funcionamiento.

\item[Deep Learning]: Es un subcampo del \textit{Machine Learning} que usa Redes Neuronales Multicapa para simular el funcionamiento de la toma de decisiones del cerebro humano en un ordenador.

\item[FLOPS]: Los \textit{FLOPS} son una medida de rendimiento que mide el número de operaciones en coma flotante por segundo que un ordenador puede hacer. FLOPs son el número de operaciones en coma flotante que un programa necesitar ejecutar para terminar con su función.

\item[IoU]: \textit{Intersection over Union} es una métrica de evaluación en tareas de detección de objetos calculada a partir de dividir el área de intersección de la \textit{bounding box} predicha y la real entre el área de unión de las dos \textit{bounding boxes}.

\item[Machine Learning]: Es una categoría de la Inteligencia Artificial que trata de desarrollar algoritmos descubran patrones recurrentes en ciertos conjuntos de datos con el objetivo de empeñar tareas específicas.

\item[mAP]: \textit{Mean Average Precision} es la métrica más utilizada en la tarea de la detección de objetos. Se calcula como la media ponderada de las AP de cada una de las clases que se utilizan en el problema.

\item[OCR]: \textit{Optical Character Recognition} es una tecnología que convierte imágenes que contienen algún tipo de texto escrito a máquina o a mano a caracteres encodificados que una máquina puede entender, leer y modificar.

\item[One-stage Detector]: Los detectores de una sola fase son una clase de modelos de \textit{Deep Learning} diseñados para ser más eficientes y veloces que sus contrapartes de dos etapas. La segunda diferencia clave con estos últimos es la fusión de las acciones de detección y clasificación en una sola pasada en la red neuronal.

\item[Overfitting]: Problema en \textit{Machine Learning} en el que el modelo que se está entrenando se sobreajusta demasiado al conjunto de datos. En vez de saber generalizar para cualquier conjunto de datos, el modelo intenta memorizar directamente los resultados. Se puede resolver aumentando el conjunto de datos y/o aplicando \textit{Data Augmentation} al dataset existente o con ciertos métodos de regularización.
	
\item[YOLO]: Siglas de \textit{You Only Look Once}, es un modelo de detección de imágenes y segmentación de objetos desarrollado en la Universidad de Washington por Joseph Redmon y Ali Farhadi \footnote{Definición extraída de \url{https://docs.ultralytics.com/es/}}. Lanzado en 2015, rápidamente ganó popularidad debido a su alta precisión y rendimiento, llevando al modelo a ser constantemente actualizado y revisado hasta la actualidad, donde se está desarrollando la versión YOLOv12.
...
La Inteligencia Artificial se está implementando actualmente en cualquier trabajo o proyecto sin ningún tipo de control y sin importar si es realmente necesario. Un ejemplo claro de una implementación beneficiosa es el artículo de investigación de Gina Belmonte et Al.\cite{BELMONTE2025103154} en el que entrenan un modelo para segmentar tejido cerebral, concretamente para aplicaciones de neuroimagen y radioterapia. Debido al crecimiento de la industria de coches autónomos, surge otro claro ejemplo de beneficio muy importante: detección e identificación de vehículos en tiempo real, donde unos sensores se encargan de detectar otros vehículos en las proximidades para poder razonar y actuar gracias a esta informacion. Un caso claro puede ser que, al aparcar, un vehículo pueda detectar y medir distancias a los vehículo próximo. Y no sólo es importante para los coches autónomos, también lo es para:
\begin{itemize}
    \item \textbf{Control del tráfico:} Se puede implementar un programa para que detecte y cuente la cantidad y tipo de vehículos de una intersección para variar dinámicamente el estado de los semaforos.
    \item \textbf{Coches robados:} Simplemente se puede hacer una deteccion de los coches y compararlos uno a uno con una base de datos de vehículos robados y poder realizar avisos a las autoridades. Aquí también se incluyen casos en los que el coche no esté en las condiciones correctas de circulación (ITV no renovada, por ejemplo).
\end{itemize}

Y estos 2 casos son particularmente el propósito de este trabajo: ambas tareas requieren la atención constante de experto y/o sistemas electrónicos automatizados (como puede ser en el caso de los semáforos, ya que la mayoria de ellos simplemente funcionan a base de un temporizador) que analicen uno a uno los vehículos en busca de irregularidades.

Este tipo de programas son ya una realidad muy extendida, con múltiples investigaciones y proyectos en partes del mundo variadas como China o India para algunas de las implementaciones de detección de vehículos y/o matrículas u otros análisis del tráfico. Un ejemplo claro puede ser el caso del proyecto de Sarvesh Shirulkar et Al.\cite{10984828} con YOLO para controlar las senales de tráfico según las detecciones del modelo YOLO. Otro ejemplo es el de Anning Ji y Xintao Ma\cite{JI2025804}, en el que utilizan como base un modelo YOLO10 al que se le modifica la parte de extraccion de características. Aunque estos modelos predominen la elección de los investigadores, hay otras opciones con diferentes arquitecturas que pueden ser muy útiles en estos mismos escenarios, como puede ser la red neuronal de Leyre Encio et Al.\cite{articleEncio} basada en CenterNet, que trata de reproducir un diseño similar a la de los transformadores con su codificador-decodificador.

\section{Objetivos}

El objetivo principal de este proyecto es la realización de un programa que pueda ayudar en gran medida a la gestión del tráfico urbano mediante la detección, reconocimiento y lectura de vehículos y matrículas. Este sistema debe realizar estas tareas en un tiempo ínfimo (en cuestión de centésimas de segundo) y con una gran precisión y, es por ello que se han analizado diversas arquitecturas, entrenándose con diferentes configuraciones, logrando así los mejores resultados posibles.

Los objetos secundarios han sido:
\begin{itemize}
    \item Analizar el estado del arte en la materia de detección y reconocimiento de vehículos y matrículas, así como analizar las herramientas de lectura óptica para leer matrículas.

    \item Seleccionar y preparar distintos conjuntos de datos con anotaciones representativos para estas tareas.
    
    \item Integrar las tareas de detección y lectura óptica en un mismo flujo capaz de realizar inferencias completas a partir de imágenes, vídeos o cámaras.
    
    \item Diseñar una aplicación web sencilla e intuitiva con la librería Streamlit que permita al usuario cargar un archivo o activar cámaras para obtener predicciones visuales y textuales.
    
    \item Comparar el rendimiento de las configuraciones finales de los modelos utilizando métricas estándar de detección y reconocimiento como mAP50, mAP50-95, precision, recall y F1-score.
...
La investigación previa al proyecto se nombra en el capítulo \ref{Estado del arte} o \textbf{Estudio del arte}, con varios de los métodos usados a lo largo de los últimos años para resolver el problema de la detección de objetos enfocado específicamente a detectar vehículos y matrículas.

Los pasos y procesos que se han seguido en el proceso de experimentación se relatan en el capítulo \ref{Metodología experimental} o \textbf{Metodología experimental}, desde la selección de datos y modelos hasta las pruebas llevadas a cabo con múltiples configuraciones.

Para poder comparar la eficacia los experimentos anteriores realizados con otros proyectos anteriores, en el capítulo \ref{Resultados experimentales} o \textbf{Resultados experimentales} se muestran las comparativas entre el rendimiento de las arquitecturas usadas, usando varias métricas y gráficas como la velocidad y las probabilidades de detección y clasificación. Además, se muestra la solución y los motivos de esta elección.
...
Las normativas y licencias aplicables a este proyecto se comentan en el capítulo \ref{Legislación y normativa} o \textbf{Legislación y Normativa}, como leyes de aplicaciones web o licencias de los modelos y datasets utilizados durante el desarrollo.

Todo trabajo lleva incorporado un desarrollo, así que en el capítulo \ref{Gestión de proyecto software} o \textbf{Gestión del proyecto software} se habla de la gestión que se ha llevado a cabo en el proyecto, los posibles riesgos que se pudiesen producir (o que se han producido) y el coste de desplegar este mismo proyecto en un entorno real.

Por último, en el capítulo \ref{Conclusión} o \textbf{Conclusión} se discuten los problemas encontrados durante el desarrollo del proyecto, las posibles mejoras aplicables, opiniones personales sobre este trabajo y agradecimientos a las personas que me han ayudado, directa o indirectamente, para realizar este proyecto.
...
En la mayoría de artículos de investigación, los investigadores usan alguno de los tantos modelos YOLO existentes, principalmente porque este tipo de modelos destaca gracias a su flexibilidad y facilidad de implementar y además son especialmente prominentes a dar buenos resultados no solo en las tareas de deteccion y reconocimiento tanto de vehículos como de matrículas, sino en cualquier tarea de detección de objetos
...
\centering
    ...
    \footnotesize{Fuente: \textit{https://www.sciencedirect.com/science/article/pii/S1077314220300035}}}
...
\begin{figure}[htb]
\centering
    \includegraphics[width=.85\textwidth]{figures/ccpd_example.png}
    \caption{Ejemplo del dataset CCPD\\
    \footnotesize{Fuente: \textit{https://github.com/detectRecog/CCPD}}}
...
Ya que los datasets tienen por defecto una estructura diferente a la pedida por los modelos YOLO y/o COCO, se han creado varios programas sencillos que reconstruyen los conjuntos de datos, creando unos nuevos, de forma que cada uno de los dataset quede con la siguiente estructura:
...
Debido a la gran cantidad de datos que existen, tanto para detección de vehículos como para detección de matrículas, se han utilizado varios datasets para cada tarea, de forma que, aunque no estén los conjuntos de entrenamiento, validación y pruebas balanceados entre sí, tengan muchos datos para cada una de las tareas. Por eso es que cada dataset tendrá un propósito detallado a continuación:
\begin{itemize}
    ...
    \item \textbf{UA-DETRAC:} Este dataset es uno de los dos escogidos para probar y obtener los resultados experimentales de los modelos de detección de vehículos. Se ha seleccionado debido a que al estar construido sobre vídeos, se pretender valorar el rendimiento de los modelos a seguir detectando los mismos vehículos durante la secuencia de imágenes.
    
    \item \textbf{UC3M-LP:} Se usará para entrenar y validar los modelos de detección de matrículas.
    
    \item \textbf{UC3M-VRI:} Segundo dataset para probar y obtener los resultados experimentales de los modelos de detección de vehículos. En este caso, se pretende medir la eficicacia de los modelos ante diferentes perspectivas.
    
    \item \textbf{CCPD:} Es el único que se usará para obtener los resultados de los modelos de detección de matrículas.
\end{itemize}

Cada dataset tiene que tener como mucho 3 carpetas, para los conjuntos de entrenamiento, validación y pruebas, y un archivo .yaml con las rutas absolutas (el uso de rutas relativas han dado problemas) a cada conjunto. En cada carpeta, hay otras 2: una para las imágenes (images) y otra para las etiquetas (labels). Cada imagen (no importa el formato) tiene que tener asociada una etiqueta en la otra carpeta con el mismo nombre con formato .txt y que contiene una línea con 5 números: \textbf{<class\_id> <center\_x> <center\_y> <width> <height>}, correspondientes al número identificador de la clase, las coordenadas x e y del centro de la \textit{bounding box} y el ancho y altura de esta última. Salvo el identificador de clase, los demás valores están normalizados al rango [0,1].

Esta estructura es la requerida para YOLO, pero el modelo RT-DETR requiere una estructura COCO (ya que este modelo fue entrenado en este dataset). Sin embargo, no es necesario ninguna modificación completa al dataset. Únicamente se tiene que añadir un archivo \textbf{.json} a cada conjunto que se puede crear directamente con el paquete yolococo a partir de las etiquetas yolo. El comando descrito a continuación contiene varios argumentos:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cfg}: Archivo de configuración del entrenamiento con formato \textit{.yaml}. En este archivo se fijan los parámetros de entrenamiento que vienen en esta lista a continuación, incluida la tasa de aprendizaje o el Data Augmentation.
    ...
    \item \textbf{Momentum}: Factor de momento para los optimizadores Adam y SGD, que influencia la incorporación de anteriores gradientes.
    ...
    \item \textbf{Label\_smoothing}: Suavizado de etiquetas, para ``lijar`` el valor de pérdida y que el modelo no tenga un exceso de confianza en las predicciones.
\end{itemize}
...
\begin{itemize}
    \item \textbf{output\_dir}: Ruta donde se archivan los modelos entrenados y los archivos con la configuración del entrenamiento.
...
\begin{itemize}
    ...
    \item \textbf{ColorJitter}: Aleatoriamente cambia el brillo, contraste, saturación y el tono de la imagen.
    ...
    \item \textbf{MotionBlur}: Aplica desenfoque de movimiento a la imagen usando un \textit{kernel} direccional, simulando efectos causados debido al movimiento de la cámara o de otros objetos durante la toma de una imagen.
    \item \textbf{Perspective}: Aplica 4 transformaciones a las esquinas de la imagen.
    \item \textbf{RandomBrightnessContrast}: Aleatoriamente cambia el brillo y contraste de la imagen.
...
El desarrollo de todo el código se ha realizado en un portátil MSI GF63 Thin 9SC con el sistema operativo Windows 11 con un procesador Intel Core i7-9750-H. La GPU es una tarjeta gráfica \textbf{NVIDIA GTX 1650} con \textbf{16 GB} de \textbf{RAM} y \textbf{4 GB} de \textbf{VRAM}.
...
Para probar el programa en un entorno real, se ha desplegado una aplicación web sencilla que procesa imágenes, vídeos o cámaras web en vivo. Toda imagen o fotograma se envía a los modelos de detección para que sea inferida y se devuelva la imagen o fotograma con cajas delimitadoras alrededor del vehículo y la matrícula. Además, fuera del cuadro del vehículo se muestra la clase del vehículo detectado y la confianza de que lo sea. En vez de esta información, fuera del cuadro de la matrícula se muestra la matrícula. El programa desarrollado tiene varias opciones de procesamiento:
\begin{itemize}
    ...
    \item \textbf{Cámara}: Se extraen los fotogramas de la cámara principal del sistema para procesarse y mostrarlos ya con las inferencias hechas continuamente y mientras la cámara esté conectada al sistema.
    \item \textbf{Multi-cam}: Función más extendida que la de una sola cámara, pues en este caso son 4 el número de cámaras que se pueden procesar, como máximo. Además, tienen otras funciones adicionales: (1) poder activar o desactivar la predicción en aquellas cámaras que se consideren oportunas y (2) generar un informe con los datos asociados a la correspondiente cámara de lugar, tiempo y fecha.
\end{itemize}

Existen 2 deslizadores para cambiar el umbral de confianza de cada modelo, lo que determina si se dibuja la caja delimitadora alrededor de un vehículo o matrícula. Este umbral está comprendido entre 0 y 1 (lo que se traduce a porcentajes: "El modelo tiene la confianza de un 80\% de que esto es un coche") y se puede cambiar en intervalos de 0.05, aunque por defecto se fija en 0.7. 

Otra implementación es una lista de matrículas que se revisa cada vez que se detecta y lee por parte de alguna de las funciones. A la lista se le pueden añadir y eliminar matrículas a conveniencia. Si la matrícula leída por el motor OCR se encuentra en la lista, saltará un aviso notificándolo.
...
En el segundo caso se selecciona la opción de cámara en la barra lateral. En este caso lo que se muestra es la cámara en vivo: se pasan los fotogramas de la cámara al programa, saca las detecciones y las dibuja en la imagen, la cual se muestra en la pantalla, tal y como se muestra en la figura \ref{fig:caso2_1}.
...
La lista de matrículas se abre pulsando sobre el botón en la barra lateral. Se mostrará una ventana como la de la figura \ref{fig:caso5_1}, la cual mostrará dos botones y una lista de matrículas, las cuales tienen cada una un botón a la derecha para eliminarlade la lista. El primer botón abrirá un desplegable, visto en la figura \ref{fig:caso5_2}, donde se podrá introducir la matrícula para añadir a la lista. El segundo botón guardará los cambios realizados en la lista de matriculas en un archivo persistente. Se puede visualizar en la figura \ref{fig:caso6_1} la ventana que se muestra cuando en una imagen o fotograma se detecta alguna de las matrículas de la lista.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{1\textwidth}{| c | >{\centering\arraybackslash}X |}
        ...
        \hline
        RF10 & El sistema debe permitir activar y desactivar las inferencias en las cámaras que se necesite en la función de múltiples cámaras.\\
        \hline
    \end{tabularx}
    \label{Req-Func}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{1\textwidth}{| c | >{\centering\arraybackslash}X |}
        ...
        \hline
        RNF2 & El tiempo de inferencia por imagen debe ser inferior a 100ms en GPU.\\
        \hline
...
\begin{enumerate}
    \item \textbf{Estudio inicial}: Aquí se hace todo el estudio previo sobre el estado del arte. La detección de vehículos y/o matrículas junto con el reconocimiento de estas últimas es un campo muy ampliable en el que montones de investigadores han realizado trabajos y proyectos 
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        & \textbf{totales} & \textbf{por hora} & & \textbf{seguridad} & \textbf{empleador (2)}\\
        & & & & \textbf{social (1)} & \\
        \hline
        Ingeniero IA & 140h & 30.26€/h & 4236.4€ & 266.89€ & 999.79€\\ 
        \hline
        Desarrollador Web & 50h & 27.60€/h & 1380€ & 86.94€ & 325.68€\\
        \hline
...
\begin{table}[ht]
    ...
    \begin{tabularx}{1\textwidth}{| >{\centering\arraybackslash}X | >{\centering\arraybackslash}X |}
        ...
        \hline 
        \textbf{Causa} & Sobrepasar el uso de utilización de la GPU o errores CUDA. Uso intensivo de la gráfica durante el entrenamiento. Limpieza de memoria de GPU o CPU no realizada después del entrenamiento o pruebas.\\
        \hline 
        \textbf{Estrategia de mitigación} & Revisión continua del porcentaje de uso de la GPU durante y posterior al entrenamiento y pruebas de los modelos. Limpieza manual de la VRAM de la GPU en casos donde no se haya gestionado correcta y automáticamente.\\
        \hline
    \end{tabularx}
    \label{R2}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{1\textwidth}{| >{\centering\arraybackslash}X | >{\centering\arraybackslash}X |}
        ...
        \hline 
        \textbf{Estrategia de mitigación} & Script para la revisión automática para revisar el dataset entero en busca de errores en las etiquetas, imágenes y/o imágenes.\\
        \hline
    \end{tabularx}
    \label{R3}
\end{table}
...
\begin{itemize}
    ...
    \item Problemas de aprendizaje del modelo RT-DETR: Este modelo necesita la modificación de muchos más hiperparámetros en comparación con otros modelos más comunes de detección de objetos y además son necesarias muchas iteraciones al conjunto de datos para que solamente comience a aprender sobre la tarea en la que se está haciendo el trabajo.
...
\begin{itemize}
    ...
    \item Añadir un nuevo modelo específico para marcas de vehículos o modificar el anterior detector de vehículos para entrenarse en conjuntos de datos donde la etiqueta (o una de ellas) sea la marca y/o modelo del vehículo.
