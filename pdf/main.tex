...
\textbf{DINO-DETR}\cite{zhang2022dino} \textit{(Improved DeNoising Anchor Boxes\footnote{Los detectores \textit{anchor-based} dividen la imagen en una red, y en cada celda de la red se generan varias cajas de anclaje para tener en cuenta múltiples formas. El contrario sería un detector \textit{anchor-less} como YOLO26, que predice directamente el centro del objeto y el tamaño usando sólo el mapa de características.})} es el segundo detector de objetos mejora previos modelos DETR en términos de eficiencia y rendimiento. Este alcanza un \textbf{49.4\%AP} en 12 iteraciones y un \textbf{51.3\%AP} en el dataset COCO con un extractor de características ResNet-50 y características multiescala. 

La base de este algoritmo (DETR) es un detector basado en un Transformador que modela la detección de objetos como una tarea de predicción y asigna las etiquetas por emparejamiento de grafos bipartitos\footnote{Forma de emparejar vértices de dos conjuntos diferentes de forma que cada vértice esté conectado por un borde a 1 solo vértice del otro conjunto.}. 
DINO es una arquitectura extremo a extremo \textit{(end-to-end)} que contiene un extractor de características, un codificador y un decodificador de un transformador de múltiples capas y múltiples cabezas de predicción; esta arquitectura se puede observar en la figura \ref{fig:dino-detr}. Siguiendo el modelo DAB-DETR\cite{liu2022dabdetrdynamicanchorboxes}, se implementan unas colas en el decodificador como cajas de anclaje dinámicas y se refinan a lo largo de las capas del decodificador. Del modelo DN-DETR\cite{li2022dndetracceleratedetrtraining} se añaden etiquetas y cajas reales con ruido en el decodificador para ayudar a estabilizar el emparejamiento por bipartición durante el entrenamiento.

\begin{figure}[htb]
\centering
    \includegraphics[width=.80\textwidth]{figures/dino-detr.png}
    \caption{Estructura de la arquitectura DINO-DETR, donde las K mejores características del codificador se utilizan para inicializar las colas posicionales del decodificador y las colas de contenido como parámetros aprendibles.\\
    \footnotesize{Fuente: \textit{https://arxiv.org/pdf/2203.03605}}}
    \label{fig:dino-detr}
\end{figure}

Las mejoras se sitúan en el transformador, tanto con el codificador como con el descodificador:
\begin{itemize}
    \item \textbf{Las K mejores características del codificador de la última capa se seleccionan para inicializar las colas posicionales para el decodificador}, mientras que las colas de contenido se mantienen como parámetros con posibilidad de aprendizaje. 
    \item \textbf{Contrastive DeNoising Training:} El codificador contiene una parte \textbf{CDN} \textit{(Contrastive Denoising)} con dos colas, positiva y negativa como se puede observar en la figura \ref{fig:cdn}, y dos hiperparámetros $\lambda_1$ y $\lambda_2$. Las colas positivas con un ruido menor a $\lambda_1$ se esperan que reconstruyan su caja. Las colas negativas con un ruido mayor que $\lambda_1$ y menor que $\lambda_2$ se predicen como "no objeto". Este método selecciona los \textit{anchors} o colas de mayor calidad para predecir las \textit{bounding boxes}, evitando el solapamiento de múltiples \textit{bounding boxes} para un mismo objeto y eliminando \textit{GT boxes} alejadas del objeto al que pertenecen.
    \item \textbf{Mixed Query Selection:} \textbf{Se inicializan las cajas de anclaje usando la información de posición asociada con las K características seleccionadas de la última capa del codificador}, manteniendo las colas estáticas de contenido generadas por una transformación lineal de las características seleccionadas de la última capa del codificador. La estructura de esta implementación se puede observar en la figura \ref{fig:mixed-query}.
    \item \textbf{Look Forward Twice:} Nueva propuesta de predicción de \textit{bounding boxes}, donde \textbf{las actualizaciones de cada capa se influencian de las pérdidas de la misma capa y de la siguiente capa} (una predicción de la siguiente caja)\footnote{Las actualizaciones se sacan del modelo Deformable DETR, donde cada valor está comprendido entre 0 y 1. Dadas 2 cajas, se aplica la función sigmoidal inversa, se suman y se transforma la suma con otra función sigmoidal.}. Esta estructura se puede observar en la figura \ref{fig:lf-twice}, en comparación con la anterior propuesta de predicción de \textit{bounding boxes} del modelo Deformable DETR\cite{zhu2021deformabledetrdeformabletransformers}.
\end{itemize}

\begin{figure}[htb]
\centering
    \includegraphics[width=.80\textwidth]{figures/cdn.png}
    \caption{Estructura de la red CDN y sus grupos. Asumiendo que el centro del cuadrado es una GT \textit{(ground truth) box}, los puntos dentro del cuadrado interior se consideran positivos y los que se encuentran entre el interior y el exterior se consideran negativos.\\
    \footnotesize{Fuente: \textit{https://arxiv.org/pdf/2203.03605}}}
    \label{fig:cdn}
\end{figure}
...
\begin{figure}[htb]
\centering
    \includegraphics[width=.80\textwidth]{figures/dino-fw.png}
    \caption{Estructura del modelo DINO\\
    \footnotesize{Fuente: \textit{https://arxiv.org/pdf/2203.03605}}}
    \label{fig:dino-fw}
\end{figure}
...
\begin{figure}[htb]
\centering
    \includegraphics[width=.80\textwidth]{figures/mixed-query.png}
    \caption{Estructura del encoder-decoder del modelo DINO que implementa una selección mixta de colas\\
    \footnotesize{Fuente: \textit{https://arxiv.org/pdf/2203.03605}}}
    \label{fig:mixed-query}
\end{figure}

\begin{figure}[htb]
\centering
    \includegraphics[width=.80\textwidth]{figures/lf-twice.png}
    \caption{Comparación de actualización de cajas entre Deformable DETR y DINO-DETR\\
    \footnotesize{Fuente: \textit{https://arxiv.org/pdf/2203.03605}}}
    \label{fig:lf-twice}
\end{figure}
...
Ya que el modelo YOLO necesita un estructurado concreto, lo primero es construir programas y/o funciones personalizadas que modifiquen la estructura de los conjuntos de datos utilizados para entrenar y validar los modelos.

Los modelos YOLO requieren que un archivo con etiquetas para cada imagen con varias líneas (una por objeto) con el formato \textbf{<class\_id> <center\_x> <center\_y> <width> <height>}, correspondiendo al número identificador de la clase (número entero comprendido entre 0 y el número de clases menos 1), las coordenadas x e y del centro de la \textit{bounding box} y el ancho y altura de esta última. Estos cuatro últimos valores están normalizados al rango [0,1].
...
En los experimentos realizados se ha prestado especial atención a la diferencia entre las pérdidas tanto de entrenamiento como de validación ya que dependiendo del diferencial entre ellas podría llevar al sobreajuste. Estas pérdidas se extraen de un archivo csv creado automáticamente por el entrenamiento del modelo YOLO, que, a cada iteración extrae las siguientes métricas: \textbf{epoch, time, train/box_loss, train/cls_loss, train/dfl_loss, metrics/precision(B), metrics/recall(B), metrics/mAP50(B), metrics/mAP50-95(B), val/box_loss, val/cls_loss y val/dfl_loss}. Aunque es muy útil seguir la evolución de las métricas como precision, recall o mAP50-95, para rastrear signos de overfitting o underfitting. En la figuras \ref{fig:g-ms} y \ref{fig:g-no-ms} se pueden ver dos configuraciones diferentes de entrenamiento para la detección de vehículos: en la primera figura se utiliza el parámetro \textit{multiscale}, que redimensiona las imágenes por cierto valor durante el entrenamiento, mientras que en la segunda no se utiliza este parámetro. De valores similares, ambas gráficas dotan de un problema: el underfitting; en ambas gráficas la pérdida de validación se mantiene inferior a la de entrenamiento, lo que significa que el modelo tenía demasiada penalización durante el entrenamiento. 
...
Este repositorio incluirá, además del código necesario para la ejecución del sistema, un \textbf{README} detallado con información sobre la implementación y la guía de instalación de:
\begin{itemize}
    \item Entrenamiento, validación y pruebas del modelo YOLO.
    \item Despliegue de la aplicación web para la detección de vehículos.
\end{itemize}
